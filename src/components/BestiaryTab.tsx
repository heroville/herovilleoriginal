...
export default function BestiaryTab() {
  ...
  const monKey = (typeof sorting.monList === 'string' ? sorting.monList : null) || 'value';
  const bossKey = (typeof sorting.bossList === 'string' ? sorting.bossList : null) || 'value';
  const monsters = orderBy(state.monsters || [], monKey);
  const bosses = orderBy(state.bosses || [], bossKey);
  const dungeons = state.dungeons || [];

  const getDungeonNamesForBoss = (bossId: number) => {
