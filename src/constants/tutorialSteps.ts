...
interface PartialFlatState {
  buildings?: Array<{ count?: number; enabled?: boolean }>;
  heroList?: unknown[];
  upgrades?: Array<{ purchased?: boolean }>;
  gold?: number;
  resources?: number;
  potion?: { count?: number; working?: number };
  weapons?: Array<{ count?: number; working?: number }>;
  blueprints?: Array<{ cost?: number }>;
  dungeons?: unknown[];
}

/**
 * Returns the tutorial step index implied by current game state.
 * Used to skip ahead when the player has already done the goal of earlier steps.
 */
export function getTutorialStepFromState(flat: PartialFlatState): number {
  if (!flat) return 0;
