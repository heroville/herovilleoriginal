...
import type { EconomyState } from '../../types/index.ts';
...
const economySlice = createSlice({
  ...
  reducers: {
    ...
    setResources(state, action: PayloadAction<number>) {
    ...
    setMaxResources(state, action: PayloadAction<number>) {
      const v = Number(action.payload);
